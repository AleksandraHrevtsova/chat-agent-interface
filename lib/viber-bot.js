...
function ViberBot(loggerOrConfiguration, configuration) {
  ...
  if (!_.isEmpty(missingFields)) {
    throw new Error(
      `Invalid configuration ${configuration}. Missing fields: ${missingFields}`
    );
  }
  ...
  this._client = new ViberClient(
    ...
    this,
    API_URL,
    configuration.registerToEvents || SUBSCRIBED_EVENTS
  );
  this._middleware = new Middleware(
    this._logger,
    new MessageValidator(this._logger, this.authToken)
  );
...
ViberBot.prototype.getUserDetails = function(userProfile) {
  return this._client
    .getUserDetails(userProfile.id)
    .then(response => Promise.resolve(response.user));
};
...
ViberBot.prototype.setWebhook = function(url, isInline, sendName, sendPhoto) {
  return this._client.setWebhook(url, isInline, sendName, sendPhoto);
};

ViberBot.prototype.sendMessage = function(
  optionalUserProfile,
  messages,
...
ViberBot.prototype.postToPublicChat = function(senderProfile, messages) {
  return this._sendMessages(
    senderProfile,
    messages,
    REQUEST_TYPE.POST_TO_PUBLIC_CHAT
  );
};

ViberBot.prototype._sendMessages = function(
  userProfile,
  messages,
  requestType,
  optionalTrackingData,
        ...
        new Promise((resolve, reject) => {
          if (requestType == REQUEST_TYPE.SEND_MESSAGE)
            return self
              ...
              .then(response =>
                resolveCallback(response, message, resolve, reject)
              );
        ...
        new Promise((resolve, reject) => {
          if (requestType == REQUEST_TYPE.SEND_MESSAGE) {
            return self
              ...
              .then(
                response =>
                  resolveCallback(response, lastMessage, resolve, reject),
                error => reject(error)
              );
          }
          if (requestType == REQUEST_TYPE.POST_TO_PUBLIC_CHAT) {
            return self
              ._sendMessageToPublicChat(
                userProfile,
                lastMessage,
                lastMessage.minApiVersion
              )
...
ViberBot.prototype._sendMessageToPublicChat = function(
  senderProfile,
  message,
  optionalMinApiVersion
) {
  const jsonMessage = message.toJson();
  return this._client.postToPublicChat(
    senderProfile,
    jsonMessage.type,
    jsonMessage,
    optionalMinApiVersion
...
ViberBot.prototype._setupTextMessageReceivedHandler = function() {
  const self = this;
  this.on(
    EventConsts.MESSAGE_RECEIVED,
    (message, response, silent, replyType, chatId) => {
      if (!(message instanceof TextMessage)) return;

      const callback = this._regexMatcherRouter.tryGetCallback(message.text);
      if (!callback) {
        return self._logger.debug(
          `Could not find regex matcher for ${message.text}`
        );
      }
...
ViberBot.prototype._setupConversationStartedHandler = function() {
  const self = this;
  this.on(
    EventConsts.CONVERSATION_STARTED,
    (response, isSubscribed, context) => {
        ...
        callback(
          ...
          (responseMessage, optionalTrackingData) => {
            ...
            if (!(responseMessage instanceof Message)) {
              throw new Error(
                "Response from conversation started callback must be message or null"
              );
            }

            const jsonMessage = responseMessage.toJson();
            if (responseMessage.keyboard)
              jsonMessage.keyboard = responseMessage.keyboard;
            if (responseMessage.trackingData) {
              jsonMessage[
                "tracking_data"
              ] = self._client._serializeTrackingData(
                optionalTrackingData || {}
              );
            }

            self._logger.debug(
              "Sending conversation started callback",
              jsonMessage
            );
