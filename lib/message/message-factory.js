...
function MessageFactory(logger) {
  ...
  _.each(
    ...
    messageType => (self._mapping[messageType.getType()] = messageType)
  );
}
...
MessageFactory.prototype.createMessageFromJson = function(json) {
  ...
  if (!_.has(this._mapping, messageType)) {
    this._logger.debug(
      `Could not build message from type ${messageType}. No mapping found`
    );
    return;
  }
  return this._mapping[messageType].fromJson(
    json.message,
    json.timestamp,
    json.message_token
  );
