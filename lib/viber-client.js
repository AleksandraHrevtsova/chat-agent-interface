...
ViberClient.prototype.setWebhook = function(
  url,
  isInline,
  sendName,
  sendPhoto
  ...
  this._logger.info(
    "Sending 'setWebhook' request for url: %s, isInline: %s, sendName: %s, sendPhoto: %s",
    url,
    isInline,
    sendName,
    sendPhoto
...
ViberClient.prototype.sendMessage = function(
  ...
  if (!optionalReceiver && !optionalChatId) {
    throw new Error(
      `Invalid arguments passed to sendMessage. 'optionalReceiver' and 'chatId' are Missing.`
    );
  }

  if (messageType && !messageData) {
    throw new Error(
      `Invalid arguments passed to sendMessage. 'MessageData' is Missing.`
    );
  }
  ...
  this._logger.debug(
    "Sending %s message to viber user '%s' with data",
    messageType,
    optionalReceiver,
    messageData
  );
...
ViberClient.prototype.getOnlineStatus = function(viberUserIds) {
  ...
  if (_.size(viberUserIds) > MAX_GET_ONLINE_IDS) {
    throw new Error(
      `Can only check up to ${MAX_GET_ONLINE_IDS} ids per request`
    );
  }
...
ViberClient.prototype.postToPublicChat = function(
  senderProfile,
  messageType,
  messageData,
  optionalMinApiVersion
  ...
  if (!senderProfile) {
    throw new Error(
      `Invalid arguments passed to postToPublicChat. 'senderProfile' is Missing.`
    );
  }

  if (!messageType || !messageData) {
    throw new Error(
      `Invalid arguments passed to postToPublicChat. 'MessageData' or 'messageType' are Missing.`
    );
  }
...
ViberClient.prototype._sendRequest = function(endpoint, data) {
  ...
  const dataWithAuthToken = Object.assign(
    { auth_token: this._bot.authToken },
    data
  );
  const options = {
    json: true,
